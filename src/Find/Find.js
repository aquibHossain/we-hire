...
const Find = () => {
    ...
    useEffect(()=>{
      fetch(`http://localhost:5000/find?search=${search}&&category=${category}&&areas=${areas}&&min=${min}&&max=${max}`)
      .then(res=>res.json())
   ...
   console.log(categories);
    return (
      ...
      <Grid container spacing={2} >
        {
          categories[0]?(categories[0] ? categories.map(list=><SearchList key={list._id} list={list}></SearchList>):<Typography
          variant="h1"
          sx={{fontFamily:'initial', fontWeight: "bold", mb: 10}}
        >
          No Result Found!!!
        </Typography>):<CircularProgress className='mx-auto' />
        }
</Grid>
</Container>
        </div>
